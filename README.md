...
[![Deploy on Railway](https://railway.app/button.svg)](https://railway.com/deploy/hive-agent?referralCode=LBtbQR&utm_medium=integration&utm_source=template&utm_campaign=generic)
...
```
POST /agents/run/{agent_name}' executes HIVE orchestration via 'uv run hive run /app/exports/<agent_name> ...' (with module-level fallback if needed), so orchestration happens inside HIVE in 'server.py'. 'POST /agents/create' picks a runnable template from '/app/examples' (or 'HIVE_CREATE_TEMPLATE') and rewrites metadata ('agent_name', 'goal', 'description').
